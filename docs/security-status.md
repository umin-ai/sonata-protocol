...
Both programs were deployed to Devnet on 14 September 2026 and matched the tested binaries byte for byte. The network demo completed 24 transactions, all subsequently checked as finalized and successful, including repayment and loss-bearing liquidation. See [Devnet evidence](devnet-deployment.md). Public RPC connection limits and duplicate-write responses interrupted the initial uploads; verified-buffer finalization recovered them. The deployment wrapper now supports resuming and skips matching deployed programs. These deployment checks do not constitute a cross-machine reproducible build or an external audit. Frontend wallet integration remains outstanding.
