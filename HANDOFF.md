...
| | **Launchpad identity:** Meteora partner metadata at [`J7yWq3VJ…SxJa7qVX`](https://explorer.solana.com/address/J7yWq3VJMrufqtjjzZVnHJTzBJzJrMecU4DPSxJa7qVX?cluster=devnet) names Sonata (name, website, logo) for every pool whose fee claimer is the Sonata Vault, including graduated ones, and new launches' metadata carries `createdOn: "https://sonata.umin.ai"` (§5.8). Which apps display partner metadata is not confirmed |
...
| | mNVDA as a quote asset: no Devnet mint exists, and the launch form shows it as coming soon. Eight quote tokens exist: mSPY, mQQQ, mTSLA, mMSFT, mAMZN, mMETA, mMCD and mANTHROPIC (§4). |
...
| | Indexing trades on graduated markets' DAMM v2 pools; the charts cover the bonding curve only |
| | Mainnet deployment, and the Meteora token-badge path that real stock tokens require (§7) |

Tests, run on 23 September: frontend `npm test` 92/92 (including Stock Floor maths 5, token profile validation 6, Irys encoding 2, upload rate limit 3, S3 keys 2, market data 2) and `npm run test:indexer` 3/3; protocol `node --test tests/*.test.mjs` 37/37 (credit 12, rewards 11, treasury 5, treasury Stock Floor 8, treasury registration 1); `cargo test -p credit-math` 6/6. Neither repository has CI; these run locally.
...
**What the shipped UI offers** ([`launch-settings.tsx`](https://github.com/umin-ai/sonata/blob/ca12d2d489f85f0a7eb8341d3b2c6d94ec8323a4/app/launch-settings.tsx)): a stock quote asset (mSPY, mQQQ, mTSLA, mMSFT, mAMZN, mMETA, mMCD or the pre-IPO mANTHROPIC); a graduation target of $25K, $50K or $100K from a fixed $5,000 opening, converted to the quote token at the Solana market price (falling back to 8, 12 or 18 quote units from a 2-unit opening when no usable price is available); a trading fee of 1%, 2% or 3%; the Stock Floor switch, off by default (§5.5); and an optional token profile (§5.6). The curve choices have the same shape as Embercurve's launch studio — a fixed opening, three graduation presets, three fee tiers — and Ember additionally offers four curve shapes (§8). Sonata claims no more curve choice than Ember.
...
- **Token metadata.** Launches from the app after 23 September 21:00 UTC publish a metadata JSON with `createdOn: "https://sonata.umin.ai"` ([`token-profile.ts:119`](https://github.com/umin-ai/sonata/blob/521c300a20f5c49fdac6a1e1737ae1a985902ead/lib/token-profile.ts#L119)), including launches without a profile. Without a profile the file is optional: a failed upload, or a transaction too large to carry the URI, launches with no URI ([`live-workspace.tsx:491`](https://github.com/umin-ai/sonata/blob/521c300a20f5c49fdac6a1e1737ae1a985902ead/app/onchain/live-workspace.tsx#L491)). Earlier launches without a profile carry no metadata URI.
...
|---|---|---|
| `stockroom_treasury` `GPANv5z…` | Yes — `b3e4f665…`, the partner metadata build (before that `c62dc338…`, the Stock Floor build; `8d7ad9a3…`; originally `b4111fe7…`) | `vb4pmin…` |
| `stockroom_rewards` `6u1nXj1…` | Yes — `2552f2b1…`, redeployed with the partner metadata build (previously `736f450a…`, `6195ac38…`) | `vb4pmin…` |
