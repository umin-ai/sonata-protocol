...
| | **Dollar launch targets priced from live markets.** Presets are set in US dollars ($5,000 opening; $25K / $50K / $100K graduation) and converted to the quote token when chosen; the converted amounts are the on-chain curve inputs and do not move afterwards. Default source: the real xStock (e.g. SPYx) priced by Jupiter across Solana DEX liquidity, used only with at least $250K liquidity, under 1% price impact on a $1,000 quote, and under 1% gap between Jupiter's price and that executable quote; the xStock scaled-UI dividend multiplier is applied so the price is per share. With a Pyth Pro key entitled to the feed, Pyth is used instead, but only if it is within 1% of the Solana market. Tested live on 23 September: SPY and NVDA priced from Solana (the free Pyth plan excludes them); QQQ and TSLA priced by Pyth in the after-hours session, cross-checked against Solana with gaps of 0.24% and 0.03%. Only mSPY is deployable on Devnet today. Logic in `lib/pricing/stock-price.ts` (tested); route `app/api/stock-price/route.ts` keeps keys server-side. |
...
Tests, run on 23 September: frontend `npm test` 72/72; protocol `node --test tests/*.test.mjs` 29/29 (credit 12, rewards 11, treasury 5, treasury registration 1); `cargo test -p credit-math` 6/6. Neither repository has CI; these run locally.
