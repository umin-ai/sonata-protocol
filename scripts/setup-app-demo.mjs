...
const manifest = existsSync(output)
  ...
  : {
      ...
      debtMint: cash.publicKey.toBase58(),
      ...
      creditProgram: c.credit.programId.toBase58(),
      oracleProgram: c.oracle.programId.toBase58(),
      stockDecimals: 8,
      cashDecimals: 6,
      demoPrice: 200,
...
async function send(label, ixs, signers = [admin]) {
  if (manifest.steps.some((s) => s.label === label)) return;
  ...
  for (let n = 0; n < 40; n++) {
    ...
    if (
      ...
      manifest.steps.push({ label, signature });
      writeFileSync(output, JSON.stringify(manifest, null, 2) + "\n");
      console.log(label, signature);
      return;
    }
  }
  throw Error("Confirmation pending: " + signature);
}
await send(
  "Fund disposable app authority with 0.25 Devnet SOL",
  [
...
async function mintAccount(k, space, programId) {
...
await send(
  "Create demo USD mint",
  [
    await mintAccount(cash, MINT_SIZE, TOKEN_PROGRAM_ID),
    createInitializeMint2Instruction(cash.publicKey, 6, admin.publicKey, null),
  ],
  [admin, cash],
);
await send(
...
await send(
  "Initialize fixed demo oracle",
  [c.initializeOracle()],
  [admin, oracle],
);
await send("Initialize interactive credit market", [c.initializeMarket()]);
await send("Seed 50000 demo USD of lending liquidity", [
  createAssociatedTokenAccountIdempotentInstruction(
    admin.publicKey,
  ...
  c.supply(admin.publicKey, 50000000000n),
]);
console.log(
  JSON.stringify({
    market: manifest.market,
    authority: manifest.admin,
